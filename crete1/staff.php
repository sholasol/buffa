                    ...
                    if($imgSize < 500000000){
                        if (move_uploaded_file($tmp_dir,$upload_dir.$proImage)) {
                            $dbs = dbConnect()->prepare("
                                INSERT INTO users 
                                (fname,lname,phone,dob,branch,qualification,salary,email,role,status,joined,gender, department,password)
                                VALUES
                                (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                                ");
                            $dbs->execute([$firstnm,$lname,$pnum,$dob,$branch,$qual,$salary,$email,$desg,$status,$joined,$sex,$desg,$password]);
                            if ($dbs) {
