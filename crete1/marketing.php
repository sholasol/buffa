...
if (isset($_POST['submit'])) {
	...
	$insert = dbConnect()->prepare("INSERT INTO m_activity (sales_id,cust_name,address,phone,staffid,date,sales_status, note, engr) VALUES (?,?,?,?,?,?,?, ?,?)");
	if ($insert->execute([$sales_id,$name,$address,$phone,$uid,$date,$status, $note, $engr])) {
		$tot = 0;
                
		foreach ($items as $i => $item) {
                    $tot = $tot + $total[$i];
                    
			$salesItem = dbConnect()->prepare("INSERT INTO marketing_items (product,quantity,sales_id,unitPrice) VALUES (?,?,?,?)");
        ...
        if($salesItem){
            $msg = "Your sales activity has been successfully aded";
            echo  " <script>alert('$msg'); window.location='Mactivity'</script>";
        }
	...
	function getProduct(){
		...
		$output = '';
	$sel = dbConnect()->prepare("SELECT * FROM production JOIN product_category AS pc ON product = pc.id ");
		$sel->execute();
		if ($sel->rowcount()<1) {?>
			<option selected="" disabled="">No Available Product</option>
		<?php }else{
			while ($row = $sel->fetch()) {
			 $output .='<option value="'. $row['id'] .'" data-amt="'.$row['unitPrice'].'">  '.$row['name'].' </option>';
                            ...
                            <div class="card-body">
                                <h4 class="card-title">Make Sales</h4>
                                    ...
                                    <div class="form-row">
                                        ...
                                        <div class="col-md-4 mb-3">
                                            <label for="validationCustom02">Engr/Project Mgr</label>
                                            <input type="text" name="engr" class="form-control" required>
                        ...
                        <div class="card">
                            <div class="card-body ">
                                ...
                                <div class="card-body items">
                                    ...
                                    <div class="form-group col-md-2">
                                    	<input type="text" id="unit_1" name="unit[]"  class="form-control" onkeyup="changePrice(1)">
                                    </div>
...
<!-- wysuhtml5 Plugin JavaScript -->
	...
	$('#add').click(function(){
		...
		html +="<div class='col-md-2 form-group'><input type='text' id=unit_"+row_id+" name='unit[]' class='form-control custom-input' onkeyup='changePrice("+row_id+")'   >	</div>";
