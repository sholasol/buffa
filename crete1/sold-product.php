        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                                ...
                                <div class="table-responsive m-t-40">
                                    <table id="example23" class="table table-bordered table-striped">
                                        <thead>
                                            ...
                                            <tbody>
                                            <?php
                                            $sact = dbConnect()->query("SELECT * FROM sales_activity ORDER BY id desc");
                                            if(empty($sact)){
                                                //echo 'No record in the database';
                                            }else{
                                                $i = 0;
                                               while($user = $sact->fetch()){
                                               $i++;
                                               ?>
                <tr>
                    <td><?php echo $i; ?></td>
                    <td>
                       <?php echo $user['sales_id'] ?>
                    </td>
                    <td><?php echo $user['cust_name'] ?></td>
                    <td><?php $sid = $user['staffid'];
                        $staff = dbConnect()->prepare("SELECT fname,lname FROM users WHERE id = ?");
                        $staff->execute([$sid]);
                        $f = $staff->fetch();
                ...
                ?>
                                            ...
                                            <td><span class="label label-info"><?php echo date("d-m-y h:i:s a", $user['date']) ?></span> </td>
                                            <td><a href="invoice?id=<?php echo $sales_id?>">Invoice</a></td>
                                        </tr>
                                            <?php }}?>
                                            </tbody>
                                    </table>
