            ...
            <div class="container-fluid">
                ...
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h4 class="text-themecolor"> Marketing Activities</h4>
                    </div>
                                ...
                                <div class="table-responsive m-t-40">
                                    <table id="example23" class="table table-bordered table-striped">
                                        <thead>
                                            ...
                                            <tbody>
                                            <?php
                                            $userr = dbConnect()->prepare("SELECT * FROM m_activity ORDER BY id desc");
                                             $i = 0;
                                            $userr ->execute();
                                            while($rw=$userr->fetch()){

                                                $mid = $rw['staffid'];
                                                  $stf = dbConnect()->prepare("SELECT fname, lname FROM users WHERE id =?");
                                                  $stf->execute([$mid]);
                                                  $rr = $stf->fetch();
                                                  $stfName = $rr['fname'].' '.$rr['lname'];
                                                  $i++;
                                               ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php 
                                                    ...
                                                    $getprd = dbConnect()->prepare("SELECT * FROM marketing_items WHERE sales_id = ?");
                                                    ...
                                                    while ($row = $getprd->fetch()) {
                                                        ...
                                                        $total = $total + $iprice;
                                                            ...
                                                            ?>
                                        <span class="label label-info"><?php echo $total ?></span> </td>
    <td width="30%">
        <?php 
        $sales_id2 = $rw['sales_id'];
            $getprd2 = dbConnect()->prepare("SELECT marketing_items.product,pc.name FROM marketing_items JOIN product_category AS pc ON marketing_items.product = pc.id WHERE sales_id = ? ");
        ...
        ?>
                                            ...
                                            <td>
                                                <h3 align="center" title="View"><a href="Minvoice?id=<?php echo $sales_id?>" class="icon-docs"></a> </h3>
                                            </td>
                                        </tr>
                                            <?php }?>
                                            </tbody>
