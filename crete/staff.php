                            ...
                            <div class="contact-page-aside">
                                ...
                                <div class="right-aside ">
                                    <div class="right-page-header">
                                        ...
                                        data-toggle="modal" data-target="#add-customer">New Contact</button>
                                            </div>
                                        </div>
                                    </div>
...
if (isset($_POST['submit'])) {
    if(empty($_POST['fname'])){
       ...
       elseif(empty($_POST['desg'])){
        $msg= "Desgination is required!";
        echo  " <script>alert('$msg'); </script>";
       }
        ...
        //Student Passport
                ...
                if(in_array($imgExt, $valid_extensions)){           
                    if($imgSize < 500000000){
                        if (move_uploaded_file($tmp_dir,$upload_dir.$proImage)) {
                            $dbs = dbConnect()->prepare("
                                INSERT INTO users 
                                (fname,lname,phone,dob,image,branch,qualification,salary,email,role,status,joined,gender, department,password)
                                VALUES
                                (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                                ");
                            $dbs->execute([$firstnm,$lname,$pnum,$dob,$proImage,$branch,$qual,$salary,$email,$desg,$status,$joined,$sex,$desg,$password]);
                            if ($dbs) {
...
if(isset($_GET['dmarketer'])){                          
    $id = $_GET['dmarketer'];
    $dsupp = dbConnect()->prepare("DELETE FROM users WHERE id=? ");
    if($dsupp->execute([$id])){
        echo "<script> alert('Deleted');window.location='staff' </script>";
    }
}
                ...
                if(in_array($imgExt, $valid_extensions)){           
                    if($imgSize < 500000000){
                        if (move_uploaded_file($tmp_dir,$upload_dir.$proImage)) {
                                ...
                                $sql_update=dbConnect()->prepare("UPDATE `users` SET image='$proImage',
                                fname='$_POST[fname]',
                                lname='$_POST[lname]',
                                email='$_POST[email]',
                                phone='$_POST[phone]',
                                department='$_POST[desg]', 
                                ...
                                echo  " <script>alert('$msg')</script>";
                                    ...
                                    $msg= "Error handling User update!";
                                    echo  " <script>alert('$msg')</script>";
                                }
                            }
                        }
                            ...
                            ?> <form method="POST" enctype="multipart/form-data">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <div class="col-md-12 m-b-20">
                                                                <div
                                                                    ...
                                                                    <span><i class="ion-upload m-r-5"></i>Upload User's
                                                                        Image</span>
                                    ...
                                    <div class="table-responsive">
                                        <table id="example23"
                                            ...
                                            <tbody>
                                                <?php
                                                $ct= dbConnect()->prepare("SELECT * FROM users WHERE role !='customer' AND role !='supplier' ORDER BY id DESC");
                                                $ct->execute();
                                                $i=0;
                                                while($t=$ct->fetch()){
                                                    $i++;
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a type="button" data-toggle="modal" data-target="#view-staff<?php echo $t['id'];?>"><img
                                                                src="upload/<?php echo $t['image']; ?>" alt="<?php echo $t['image'];?>" width="40" height="30"
                                                                class="img-circle" alt="<?php echo $t['image']; ?>" />
                                                    ...
                                                    <td>
                                                        <?php 
                                                           $roleid = $t['department'];
                                                           $getRole = dbConnect()->prepare("SELECT role FROM role WHERE id = ?");
                                                           $getRole->execute([$roleid]);
                                                           $fr = $getRole->fetch();
                                                        ?>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a type="button" class="icon icon-pencil text-success" data-toggle="modal" data-target="#update-staff<?php echo $t['id'];?>"> </a>
